                ...
                {history.map((item, index) => item.type === "education" ? (
                  <article className="is-learning is-education-featured" key={item.title + item.date}>
                    ...
                    <div className="resume-timeline__card">
                      ...
                      <div className="resume-education-title"><div><p>{item.eyebrow}</p><h3>{item.title}</h3></div><span>{item.sideInfo.text} {item.sideInfo.badge && <b>{item.sideInfo.badge}</b>}</span><div className="resume-education-courses">{item.tags.map((tag) => <span key={tag}>{tag}</span>)}</div></div>
                      <div className="resume-education-detail">
                        <section><p className="resume-education-label">{item.detail.label}</p><ul>{item.detail.items.map((entry) => <li key={entry}><Check />{entry}</li>)}</ul></section>
                      </div>
                      <div className="resume-education-projects"><p className="resume-education-label">{item.projects.label}</p><div>{item.projects.items.map((project, projectIndex) => <article key={project.name}><span>{String(projectIndex + 1).padStart(2, "0")}</span><p><strong>{project.name}</strong><small>{project.summary}</small></p></article>)}</div></div>
                      {item.credential && <div className="resume-education-certificate"><Award /><p><small>{item.credential.label}</small><strong>{item.credential.value}</strong></p></div>}
                    </div>
                  </article>
                ) : (
                  <article className="is-work" key={item.title + item.date}>
                    ...
                    <div className="resume-timeline__card">
                      ...
                      <div className="resume-work-title"><p>{item.eyebrow}</p><h3>{item.title}</h3><span className="resume-work-role">{item.sideInfo.text}</span><div className="resume-work-stack">{item.tags.map((tag) => <span key={tag}>{tag}</span>)}</div></div>
                      <div className="resume-work-detail">
                        <section><p className="resume-work-label">{item.detail.label}</p><ul>{item.detail.items.map((entry) => <li key={entry}><Check />{entry}</li>)}</ul></section>
                      </div>
                      <div className="resume-work-projects"><p className="resume-work-label">{item.projects.label}</p><div>{item.projects.items.map((project, projectIndex) => <article key={project.name}><span>{String(projectIndex + 1).padStart(2, "0")}</span><p><strong>{project.name}</strong><small>{project.summary}</small></p></article>)}</div></div>
                    </div>
                  </article>
                ))}
              </div>
